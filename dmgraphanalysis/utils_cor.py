...
def regress_movement_wm_csf_parameters_and_filter(data_matrix,rp,mean_wm_ts,mean_csf_ts):
    ...
    #b,a = filt.iirdesign(wp = 0.02,ws = 0.01,gpass = 0.01,gstop = 1)
    #b,a = filt.firwin2(resid.shape[0],freq =  0.02,ws = 0.01,gpass = 0.01,gstop = 1)
    ...
    for i in range(data_matrix.shape[0]):
        ...
        rpy.r.assign('r_mean_wm_ts',mean_wm_ts)
        rpy.r.assign('r_mean_csf_ts',mean_csf_ts)
        
        resid_data_matrix[i,] = rpy.r.lm(rpy.r("r_serie ~  r_rp[,1] + r_rp[,2] + r_rp[,3] + r_rp[,4] + r_rp[,5] + r_rp[,6] + r_mean_wm_ts + r_mean_csf_ts"))['residuals']
        ...
        #print stats.zscore(resid_filt)
        
    #print resid_data_matrix.shape
    
    return resid_data_matrix,resid_filt_data_matrix,z_score_data_matrix

def regress_movement_wm_csf_parameters(data_matrix,rp,mean_wm_ts,mean_csf_ts):
    ...
    for i in range(data_matrix.shape[0]):
        ...
        rpy.r.assign('r_mean_wm_ts',mean_wm_ts)
        rpy.r.assign('r_mean_csf_ts',mean_csf_ts)
        resid = rpy.r.lm(rpy.r("r_serie ~  r_rp[,1] + r_rp[,2] + r_rp[,3] + r_rp[,4] + r_rp[,5] + r_rp[,6] + r_mean_wm_ts + r_mean_csf_ts"))['residuals']
...
def regress_movement_wm_parameters(data_matrix,rp,mean_wm_ts):
    ...
    for i in range(data_matrix.shape[0]):
        ...
        rpy.r.assign('r_rp', rp)
        rpy.r.assign('r_mean_wm_ts',mean_wm_ts)
        resid = rpy.r.lm(rpy.r("r_serie ~  r_rp[,1] + r_rp[,2] + r_rp[,3] + r_rp[,4] + r_rp[,5] + r_rp[,6] + r_mean_wm_ts "))['residuals']
        
        #resid = regress_serie_movement_parameters(data_matrix[i,:],rp)
        
        ## compute Z score on the residuals 
        resid_data_matrix[i,] = stats.zscore(resid)
...
#def regress_serie_movement_parameters(serie,rp):

    
    ##rpy.r('print str(rp)')

    #lm_mod =  rpy.r.lm(rpy.r("r_serie ~  r_rp[,1] + r_rp[,2] + r_rp[,3] + r_rp[,4] + r_rp[,5] + r_rp[,6]"))['residuals']
    ##lm_mod = rpy.r('lm(r_serie ~  r_rp[,1] + r_rp[,2] + r_rp[,3] + r_rp[,4] + r_rp[,5] + r_rp[,6])$residuals')
    ##print lm_mod
    ##return lm_mod 
    
    #return rpy.r.lm(rpy.r("r_serie ~  r_rp[,1] + r_rp[,2] + r_rp[,3] + r_rp[,4] + r_rp[,5] + r_rp[,6]"))['residuals']
